...
class Laporan extends CI_Controller
{
    ...
    private function _cetak($data, $table, $tanggal)
    {
        ...
        if ($table == 'kas') :
            ...
            $pdf->Cell(77, 7, 'Keterangan', 1, 0, 'C');
            $pdf->Cell(38, 7, 'Kategori', 1, 0, 'C');
            ...
            foreach ($data as $d) {
                ...
                $pdf->Cell(77, 7, $d->keterangan, 1, 0, 'L');
                $pdf->Cell(38, 7, $d->kategori, 1, 0, 'C');
                if ($d->jumlah_masuk != 0) {
                    $pdf->Cell(25, 7,"Rp. " . number_format($d->jumlah_masuk), 1, 0, 'L');
                } else {
                    $pdf->Cell(25, 7, "-", 1, 0, 'C');
                }
                if ($d->jumlah_keluar != 0) {
                    $pdf->Cell(25, 7,"Rp. " . number_format($d->jumlah_keluar), 1, 0, 'L');
                } else {
                    $pdf->Cell(25, 7, "-", 1, 0, 'C');
                }
                $pdf->Ln();
            ...
            $pdf->Cell(145, 7, 'Jumlah', 1, 0, 'C');
            $pdf->Cell(50, 7, "Rp. " . number_format($total), 1, 0, 'C');
            $pdf->Ln();
        endif;
